...
| — | ***26–29 are targeted, not general. Do not pick one off this table to run tonight.*** *They farm a third title's corpus, and the entry point is a question you already have — "I am short of vocabulary for this family, does Modern Warfare 2019 carry it?" — not "which method next". They reach* `image` *and* `material` *only, Cold War's two best-covered pools at 81% and 76% named, and they cannot reach* `sound_alias` *at all. Run cold on 2026-08-23 they returned 412 names for 58.8 T candidates — **1 per 142.6 billion**, against 1 per 3.9 billion for the sound-alias ground the same evening. Aimed at a family you know is missing they are worth far more than that; aimed at nothing they are the most expensive seam here.* |
...
## A third title as spare parts: targeted, never a default
...
| MW19's glued `t9<token>` words dropped into Cold War's character slot | The observation is right and the use was wrong. MW19 omits the underscore after `t9` -- `t9woods`, `t9mi6` -- where Cold War writes `c_t9_usa_pl_woods_infiltration_torso`, so no underscore tokenizer here had ever seen those 519 tokens, and 262 of them are real Cold War segments. Poured into Cold War's character-model template -- 17 beginnings x 482 tokens x 3,501 tails, 28.7 M candidates -- it returned **0**. **The tokens are not a vocabulary to harvest and this row originally said they were**, claiming the glued words are "weapon behaviour" because `t9standard`, `t9accurate` and `t9fastfire` head the frequency list. That was a semantic conclusion drawn from a purely syntactic fact: gluing is how this title spells *every* `t9` compound, and weapon-stat words top the ranking only because MW19 has a great many of them. The right response to a missing separator is to **restore it** -- see method 29, which respells the corpus and returned 9 names from 1.2 M segment boundaries nothing here could previously see. |
