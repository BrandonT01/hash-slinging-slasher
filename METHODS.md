...
| 18 | heads of length k | any name that is a known one with its **first k characters** replaced | `scripts/tails.py --head` → `confirm_plan` | **692 on Cold War in one pass; 0 on Black Ops 4 over 46,371,301,910 candidates.** Spent at k=3 for the 2026-08-22 corpus |
...
| Tried | Outcome |
|---|---|
| Lowering `uncarried.py` to beginnings heading only 5–9 names in Cold War | A disjoint band of 140 real beginnings and 135,778 stems, exhaustive over **88,011,299,600 candidates** on 2026-08-22: **0 matches**. The 10–19 band produced three Cold War images, but the next rarer band is closed; lowering the family floor further spends more cross products on weaker evidence. |
| Filling speaker/voice-line grids by substituting the speaker in both directory and basename | `speaker_grids_20260822-021342.py`, measured 2026-08-22: **1,466,473 candidates, 0 matches in Black Ops 4 and 0 in Cold War**. The `<parent>/<speaker>/vox_<speaker>_<line>` relation is real and 771,122 known names fit it, but every grid cell this published corpus can imply is already named; widening the same grids repeats closed ground. |
| Lowering `uncarried.py`'s family threshold from 20 to 10 in Black Ops 4 | 321 beginnings and 149,219 stems that the committed beginning list cannot express, exhaustive over **221,773,754,370 candidates** on 2026-08-22: **0 new names**. The default threshold-20 plan remains a proven seam; the additional 10–19-name families did not extend it in Black Ops 4. |
| Replacing the first three characters of every known name in Black Ops 4 | `scripts/tails.py --head --length 3`, measured 2026-08-22: **46,371,301,910 candidates, 0 new names** against 183,932 candidate-reachable unnamed ids. The mirror direction was productive on Cold War, but this direction is exhausted for the current Black Ops 4 stems; rerun only after the corpus materially changes. |
