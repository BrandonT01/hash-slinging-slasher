...
def edits(name, deletions, transpositions, sound=False, insert=""):
...
def main(argv):
    ...
    for kind, names in work:
        insert = ""
        if args.insertions:
            insert = "0123456789" if args.digits_only else alphabet_of(names)
            print(f"{kind}: inserting [{insert}]", file=sys.stderr)

        count = 0
        batch = []
        append = batch.append
        for name in names:
            for candidate in edits(name, deletions, transpositions, args.sounds, insert):
                count += 1
                if not args.size:
                    append(candidate)
                    if len(batch) >= 65536:
