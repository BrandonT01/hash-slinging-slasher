...
def edits(name, deletions, transpositions, sound=False, insert="", substitute="", double_deletions=False):
    ...
    if transpositions:
        for i in range(len(base) - 1):
            ...
            if edited not in seen:
                ...
                yield head + edited + tail
    if double_deletions:
        # The mirror-of-a-mirror: single deletion paid (`_01` -> `_1`), so two independent
        # positions dropped at once -- `_01_` collapsing two zero-padded segments in the same
        # name -- is the same reasoning one step further, and it is still free of an alphabet.
        for i in range(len(base)):
            for j in range(i + 1, len(base)):
                edited = base[:i] + base[i + 1 : j] + base[j + 1 :]
                if edited and edited not in seen:
                    seen.add(edited)
                    yield head + edited + tail
    if substitute:
...
def main(argv):
    ...
    parser.add_argument(
        "--double-deletions",
        action="store_true",
        help="two characters dropped at once, anywhere in the name -- the mirror-extension of "
        "deletion, worth trying only because single deletion paid",
    )
    parser.add_argument(
        "--insertions",
        action="store_true",
    ...
    for kind, names in work:
        ...
        append = batch.append
        for name in names:
            for candidate in edits(name, deletions, transpositions, args.sounds, insert, substitute, args.double_deletions):
                count += 1
                if not args.size:
                    append(candidate)
                    if len(batch) >= 65536:
