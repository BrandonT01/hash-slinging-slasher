...
## Measured, and it is dead — 2026-08-22

**7 names from 96 billion candidates across both games. 1 per 13.7 billion.** Among the worst
figures ever recorded here. Kept, and kept documented, because a measured negative costs the next
person nothing and this idea is an obvious one to have.

It joins two other measurements saying the same thing: cross-type core seams returned 0 matched
ids in 190 million candidates, and this returned ~0 in 96 billion. **Asset names are not freely
recombinable.** A head constrains its tail semantically, so a head and a tail that never appeared
together mostly never will. What is productive is taking one real name and varying one thing about
it -- see `final_byte.py` at 1 per 18 and `tails.py`, which is free.

Worth running again only if the piece lists are built differently: these are capped by popularity,
and the commonest pieces are the most generic, so their crossings are the least surprising names
in the game. A version aimed at *rare* pieces within one family is a different measurement and has
not been taken.
