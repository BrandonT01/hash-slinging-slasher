...
| `harvest_bo3_tools.py` | **the Black Ops 3 mod tools**, which Steam ships beside the game and which need no format work at all: a source asset is named by its filename and a `.gdt` is a plain-text table of asset names. 248,726 files, **867,766 names**, and the largest external vocabulary on a normal disk. |
| `harvest_iwd.py` | `.iwd` is a ZIP with the extension changed, so `zipfile` lists every path inside without decompressing anything. 208 archives across the older titles, 528,740 names, **measured 0 on both games** -- kept as the measurement. |
